...
$current_min = isset( $_GET['min_price'] ) ? max( 0, (float) wc_format_decimal( wp_unslash( $_GET['min_price'] ) ) ) : $min_price;
$current_max = isset( $_GET['max_price'] ) ? max( 0, (float) wc_format_decimal( wp_unslash( $_GET['max_price'] ) ) ) : $max_price;
...
<aside class="sariyah-product-filters" aria-label="<?php esc_attr_e( 'Product filters', 'sariyah' ); ?>">
    <form method="get" action="<?php echo esc_url( is_product_category() ? get_term_link( get_queried_object() ) : wc_get_page_permalink( 'shop' ) ); ?>">
        ...
        <fieldset>
            ...
            <div class="sariyah-product-filters__prices">
                <label><?php esc_html_e( 'From', 'sariyah' ); ?><input type="number" min="0" step="0.01" name="min_price" value="<?php echo esc_attr( $current_min ); ?>"></label>
                <label><?php esc_html_e( 'To', 'sariyah' ); ?><input type="number" min="0" step="0.01" name="max_price" value="<?php echo esc_attr( $current_max ); ?>"></label>
            </div>
        </fieldset>
        ...
        <a class="sariyah-product-filters__reset" href="<?php echo esc_url( is_product_category() ? get_term_link( get_queried_object() ) : wc_get_page_permalink( 'shop' ) ); ?>"><?php esc_html_e( 'Reset', 'sariyah' ); ?></a>
    </form>
</aside>
