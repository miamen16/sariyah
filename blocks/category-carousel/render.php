...
$terms = get_terms(
    array(
        ...
        'hide_empty' => true,
        'number'     => max( 2, min( 20, (int) ( get_field( 'count' ) ?: 10 ) ) ),
        'orderby'    => 'name',
        'order'      => 'ASC',
    )
);
...
<?php foreach ( $terms as $term ) : $link = get_term_link( $term ); if ( is_wp_error( $link ) ) { continue; } $thumbnail_id = (int) get_term_meta( $term->term_id, 'thumbnail_id', true ); ?><a class="sariyah-category-carousel__item" href="<?php echo esc_url( $link ); ?>"><div class="sariyah-category-carousel__image"><?php echo $thumbnail_id ? wp_kses_post( wp_get_attachment_image( $thumbnail_id, 'woocommerce_thumbnail', false, array( 'loading' => 'lazy' ) ) ) : ''; ?></div><strong><?php echo esc_html( $term->name ); ?></strong></a><?php endforeach; ?>
</div></div></section>
